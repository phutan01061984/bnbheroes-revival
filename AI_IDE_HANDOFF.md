...
- Password-only login source commit is `f58ef837b97a172652a0ccbd41ede7ef15d8fe4a`; Production is **Ready** at `https://bnbheroes-revival.vercel.app`, immutable `https://bnbheroes-revival-4mvuz73l5-phu-tans-projects.vercel.app`, deployment `dpl_AMm1eTmz6fWXDXUnDHRXwQWAgB3d`.
- Login is intentionally simplified to **one password field only; no username**. The owner-chosen password value is intentionally **not stored anywhere in Git/HANDOFF**; it lives only as Vercel Secret `BNBH_SITE_PASSWORD` for Production. Correct login sets an HttpOnly/Secure/SameSite=Strict derived-token cookie for 7 days; the plaintext password is not stored in the cookie. Missing secret fails closed with 503.
- Live Production certification: unauthenticated `/` returns only a minimal login page with exactly one password input and no username input; wrong password returns 401 + `Sai mật khẩu`; correct password POST returns 303 + private access cookie; cookie-authenticated game/direct assets return 200. Direct asset requests without a valid cookie receive the login page rather than protected bytes. Responses use `Cache-Control: private, no-store`, `Pragma: no-cache`, and `X-Robots-Tag: noindex, nofollow, noarchive, nosnippet`. Authenticated Hero art bytes still match repository SHA-256.
...
- Economy certification is now **complete**. Corrected commit `69529de001dc3e9d7c023d9c0036b89490f3da6b` passed `npm run check:all`, `npm run economy:test`, desktop/mobile `read_image`, calculator and pool-slider browser interactions on clean Preview `https://bnbheroes-revival-oonrcw7qf-phu-tans-projects.vercel.app` / `dpl_4hfG7rzQvK37bCEkdAaSp5yAkA9z`. The first `8afdf06` Preview was rejected and never promoted.
- The already-tested Preview output was promoted. Current Production is **Ready** at `https://bnbheroes-revival.vercel.app`, deployment `dpl_2K8Qxx9GtYf4ynvB9DT1zGyaEHLh`, immutable URL `https://bnbheroes-revival-eds49batf-phu-tans-projects.vercel.app`. Public dashboard: `https://bnbheroes-revival.vercel.app/economy/`.
- Production byte certification proves `index.html`, preservation provider/shim, engine, recovered main JS/CSS and Lose art all match Git **and** frozen tag `preservation-v1.0.0`; `economy/index.html` and `economy/data.json` also match Git exactly. Dashboard interactions did not change localStorage. Durable proof: `research/browser-regression/ECONOMY_PRODUCTION_CERT_2026-09-04.md` plus the three committed economy screenshots named there.
- First economy Preview from commit `8afdf06` was **not promoted**. Browser review exposed stale human-note assumptions in the narrative: direct period table proves Arnulf ID14 is **600/700/400** (not the older 600/800/600 note), exact Training L4 is **+140 A/D/S** (not +90), and exact class labels are `Soldier/Hunter/Rogue/Mage/Knight`. The deterministic model itself was already using the restored stat arrays for aggregate output; the Hero/class/UI metadata and Arnulf explanatory example are now corrected and tests cross-check all 21 templates against `character-template-table-20260903.tsv`.
