### 2026-09-04 HTTPS Lose-art crop/alpha correction — newest safety checkpoint
...
- New runtime candidate hash is `9f54476dc22af25fa45faccf82c510f699f6e3517222152562a402275ff0e107`. Method: period Result color for visible upper Mage; first-party mapped Mage/staff alpha filtered by strong vertical continuity and geometry gating to remove horizontal source-sheet rules; warm period-pixel residual only for orange glow alpha; lower first-party Mage retained; old join repaired by RGB-only vertical interpolation inside already-proven opaque character/staff support. No generative/new character geometry.
- Immediate continuation: run `npm run forensics:all`; if PASS, curated commit/push only runtime/proof/verifier/HANDOFF/rejected archive; deploy a clean detached-worktree Preview; repeat My Heroes + Win + forced Lose browser/read_image. **Only if all three PASS**, promote that exact tested Vercel deployment to `bnbheroes-revival.vercel.app`, smoke Production, then append the final production certification and push a docs-only closeout.
