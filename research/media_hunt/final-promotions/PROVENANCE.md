...
- A fourth HTTPS Preview on commit `1132eb0` (`bnbheroes-revival-9uetl1d5o-...`) again passed My Heroes and Win but `read_image` still found a broad rectangular glow/alpha field in the forced-Lose modal. That rejected hash `9f54476dc22af25fa45faccf82c510f699f6e3517222152562a402275ff0e107` is preserved at `archive/pre-result-browser-alpha-fix-20260904/You lose.00f95b2b.png`.
- Final Lose reconstruction uses the **entire first-party Level-2 Skull Mage + staff** as one continuous layer. SIFT/RANSAC gives a simple source→Result affine geometry (21/34 inliers; warped silhouette IoU ~0.939 against the already-validated lower support). Sheet background and its horizontal rule are removed row-wise before masking; RGB is color-fit only from trusted visible period Mage pixels. The orange glow is a smooth Gaussian alpha field and contains no captured modal/background rectangle. The exact candidate hash `0e2369ac...` passed a deterministic forced-Lose `read_image` gate on temporary HTTPS Preview `bnbheroes-revival-hx2arik4p-...` before formal commit. Provenance remains reconstruction from first-party/period evidence, not original hashed bytes.
- See `research/proofs/result-art-cleanup-20260904/PROOF.md`.
...
- `You lose.00f95b2b.png`: `0e2369ac0879584ff11584fe49a682736e60a416206b8d9197a491aeec096d9f`
